...
_Config = dofile('/root/tdbot/Config.lua')
local json = (loadfile "/root/tdbot/data/JSON.lua")()
local token = '498843190:AAHptAwfB6GjrAr9-ML0o1czE0SwxQ1ckx0'
local url = 'https://api.telegram.org/bot' .. token
local offset = 0
local redis = require('redis')
local redis = redis.connect('127.0.0.1', 6379)
local SUDO = _Config.SUDO
local SUDO_ID = _Config.SUDO_ID
...
bot = dofile('/root/tdbot/data/utils.lua')
function is_sudo(user)
  local var = false
 for v,_user in pairs(SUDO_ID) do
    if _user == user then
      var = true
    end
end
        ...
        for i=1, #updates.result do
          ...
          if msg.inline_query then
            local q = msg.inline_query
						if q.from.id == _Config.cliidforpanel or q.from.id == _Config.sudoidforpanel then
            if q.query:match('%d+') then
              local chat = '-'..q.query:match('%d+')
			   local notif = redis:get('groupnotification')
	                      --'..buton..'
							local function is_lock(chat,value)
							local reanof = (redis:get('panelstatus:'..msg.chat_id) or 'off') 
local hash = SUDO..'settings:'..chat..':'..value
 if redis:get(hash) then
    return true 
    else
  ...
  if redis:get('panelstatus') == 'off' and not is_sudo(q.from.id) then 
  local keyboard = {}
  keyboard.inline_keyboard = {
							}
            answer(q.id,'settings','Group settings',chat,'مالک گرامی/مدیرگروه،\nبه درخواست مدیریت ربات دسترسی به پنل کاربری موقتا غیرفعال میباشد.',keyboard)
  elseif not redis:sismember('warnpanel:'..chat,q.from.id) and redis:get('readnoti:') == 'on' then
  redis:sadd('warnpanel:'..chat,q.from.id)
  local keyboard = {}
							keyboard.inline_keyboard = {
								{
				 {text = '🔚بستن پنل کاربری و شروع مجدد', callback_data = 'qwcloces:'..chat}
				}
							}
            answer(q.id,'settings','Group settings',chat,''..(notif or 'اطلاعیه ای ثبت نشده است.')..'',keyboard)
  elseif redis:hget('sizepanel'..chat,'changepanelsize') == 'desktop' then
...
sec = time
              local keyboard = {}
							keyboard.inline_keyboard = {
								{
                 {text = '👁‍🗨تنظیمات گروه', callback_data = 'bsd:'..chat} --,{text = 'واحد فروش', callback_data = 'aboute:'..chat}
                },{
				 {text = '👥پشتیبانی', callback_data = 'supportbot:'..chat},{text = ''..(buton or "📬تبلیغات شما")..'', callback_data = 'youradds:'..chat}
				  },{
				 {text = '💾اطلاعات گروه', callback_data = 'groupinfo:'..chat},{text = '🗒راهنما استفاده', callback_data = 'helpbot:'..chat}
				  },{
				 {text = '⚙️تنظیمات پنل', callback_data = 'poptions:'..chat},{text = '🤔نظرسنجی', callback_data = 'nazarsanji:'..chat}
				  },{
				 {text = ''..(butonalarm or "📣 اعلانات")..'', callback_data = 'buttonalarm:'..chat}
				 },{
				 {text = '🔚بستن پنل کاربری', callback_data = 'cloces:'..chat}
				}
							}
            answer(q.id,'settings','Group settings',chat,'به پنل کاربری گروه خوش آمدید.\nجهت مدیریت گروه از منوی زیر انتخاب کنید ...\n➖➖➖➖➖➖➖➖➖\nشناسه گروه شما: '..chat..'\nمیزان اعتبار گروه شما: ['..days..'] روز',keyboard)
			else
			local time = redis:ttl("charged:"..chat)
local days = math.floor(time / 86400)
time = time - days * 86400
...
sec = time
			local keyboard = {}
							keyboard.inline_keyboard = {
								{
                 {text = '👁‍🗨تنظیمات گروه', callback_data = 'bsd:'..chat} 
                },{
				 ...
				 {text = ''..(buton or "📬تبلیغات شما")..'', callback_data = 'youradds:'..chat}
				  },{
				 {text = '⚙️تنظیمات پنل', callback_data = 'poptions:'..chat}
				 },{
...
local chat = ('-'..q.data:match('(%d+)') or '')
		  if redis:get("cheaktimepanel:"..chat) then
		  Canswer(q.id,"سریع لمس میکنید،\nلطفا آهسته تر!",true)
	  else
	  redis:setex("cheaktimepanel:"..chat, 1, true)
	  end
            local q = msg.callback_query
						local chat = ('-'..q.data:match('(%d+)') or '')
         ...
         result = 'ارسال پیام خوش آمدگویی فعال گردید.'
          ...
          result = 'ارسال پیام خوش آمدگویی غیرفعال گردید!'
							...
							--------------------------------------------------
							if q.data:match('qwcloces') then
							local chat = '-'..q.data:match('(%d+)$')
							Canswer(q.id,'برای دسترسی مجدد به پنل کاربری عبارت\n/panel\nرا مجدد در گروه ارسال کنید.',true)
							end
							-------------------------------------------------------------------------
							if q.data:match('firstmenu') then
							local chat = '-'..q.data:match('(%d+)$')
							local buton = redis:get('botdikme') 
							local butonalarm = redis:get('botalarm')
							local function is_lock(chat,value)
local hash = SUDO..'settings:'..chat..':'..value
 if redis:get(hash) then
    return true 
...
sec = time
		...
		local keyboard = {}
              ...
              edit(q.inline_message_id,'`Welcome to support section.`\n*Select from the menu below:*',keyboard)
							...
							------------------------------------------------------------------------
							if q.data:match('buttonalarm') then
				local chat = '-'..q.data:match('(%d+)$')
				local alarm = redis:get('alarmtext')  
							 if alarm then
				Canswer(q.id,'🔔اعلانات مدیریت:\n '..alarm..'',true)
				else
				Canswer(q.id,'در حال حاضر اعلاناتی از طرف مدیریت ثبت نشده است.\nبا بررسی مداوم این بخش از آخرین اعلانات مطلع شوید.',true)
				end
				end
		...
		local keyboard = {}
				 ...
				 {text = '📝 راهنما قفل ها', callback_data = 'bty:'..chat}
				 },{
				 {text = '📝 راهنما قابلیت افزوده', callback_data = 'gkj:'..chat}
				 },{
			...
			------%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%---------
			if q.data:match('gkj') then
                           local chat = '-'..q.data:match('(%d+)$')
		local keyboard = {}
							keyboard.inline_keyboard = {
								{
                  {text = '🇳🇿 راهنما انگلیسی', callback_data = 'heafozde:'..chat},{text = '🇮🇷 راهنما فارسی', callback_data = 'afzofa:'..chat}
				 },{
                   {text = 'بازگشت به منوی اصلی ◀️', callback_data = 'firstmenu:'..chat},{text = 'صفحه قبلی ◀️', callback_data = 'helpsmn:'..chat}
				}
							}
              edit(q.inline_message_id,'`به بخش راهنمای قابلیت افزوده خوش آمدید`\n`از منوی زیر نوع نمایش زبان راهنما را انتخاب کنید:`',keyboard)
            end
			if q.data:match('heafozde') then
                           local chat = '-'..q.data:match('(%d+)$')
               local helpen = redis:get('hafzodeen')  
               if helpen then
               local keyboard = {}
              keyboard.inline_keyboard = {
                {
              ...
              edit(q.inline_message_id, '[راهنمای قابلیت افزوده (اصلی-فرعی)--نسخه انگلیسی]\n'..helpen..'',keyboard) 
        else
        local keyboard = {}
              keyboard.inline_keyboard = {
                {
                   {text = 'بازگشت به منوی اصلی ◀️', callback_data = 'firstmenu:'..chat},{text = 'صفحه قبلی ◀️', callback_data = 'gkj:'..chat}
        }
              }
        edit(q.inline_message_id,'`متن راهنما برای این بخش ثبت نشده است`\nبا مدیریت ربات در میان بگذارید.',keyboard)
            end
      end
	  	if q.data:match('afzofa') then
                           local chat = '-'..q.data:match('(%d+)$')
               local helpfa = redis:get('hafzodefa')  
               if helpfa then
               local keyboard = {}
              keyboard.inline_keyboard = {
                {
              ...
              edit(q.inline_message_id, '[راهنمای قابلیت افزوده (اصلی-فرعی)--نسخه فارسی]\n'..helpfa..'',keyboard) 
        else
        local keyboard = {}
              keyboard.inline_keyboard = {
                {
        ...
        edit(q.inline_message_id,'`متن راهنما برای این بخش ثبت نشده است`\nبا مدیریت ربات در میان بگذارید.',keyboard)
			...
			------------%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%---------------------------
			if q.data:match('bty') then
                           local chat = '-'..q.data:match('(%d+)$')
		local keyboard = {}
							keyboard.inline_keyboard = {
								{
                  {text = '🇳🇿 راهنما انگلیسی', callback_data = 'eqi:'..chat},{text = '🇮🇷 راهنما فارسی', callback_data = 'nqe:'..chat}
				 },{
                   {text = 'بازگشت به منوی اصلی ◀️', callback_data = 'firstmenu:'..chat},{text = 'صفحه قبلی ◀️', callback_data = 'helpsmn:'..chat}
				}
							}
              edit(q.inline_message_id,'`به بخش راهنمای قفل ها خوش آمدید`\n`از منوی زیر نوع نمایش زبان راهنما را انتخاب کنید:`',keyboard)
            end
			if q.data:match('eqi') then
                           local chat = '-'..q.data:match('(%d+)$')
               local helpen = redis:get('helplocken')  
               if helpen then
               local keyboard = {}
              keyboard.inline_keyboard = {
                {
              ...
              edit(q.inline_message_id, '[راهنمای قفل ها (اصلی-فرعی)--نسخه انگلیسی]\n'..helpen..'',keyboard) 
        else
        local keyboard = {}
              keyboard.inline_keyboard = {
                {
                   {text = 'بازگشت به منوی اصلی ◀️', callback_data = 'firstmenu:'..chat},{text = 'صفحه قبلی ◀️', callback_data = 'bty:'..chat}
        }
              }
        edit(q.inline_message_id,'`متن راهنما برای این بخش ثبت نشده است`\nبا مدیریت ربات در میان بگذارید.',keyboard)
            end
      end
	  	if q.data:match('nqe') then
                           local chat = '-'..q.data:match('(%d+)$')
               local helpfa = redis:get('helplockfa')  
               if helpfa then
               local keyboard = {}
              keyboard.inline_keyboard = {
                {
              ...
              edit(q.inline_message_id, '[راهنمای قفل ها (اصلی-فرعی)--نسخه فارسی]\n'..helpfa..'',keyboard) 
        else
        local keyboard = {}
              keyboard.inline_keyboard = {
                {
...
t = t..k..'- *['..v..']* > تعداد اخطار ها: `'..(cont - 1)..'`\n'
		...
		local keyboard = {}
							keyboard.inline_keyboard = {
								{
				 {text = '❌خیر', callback_data = 'ryt:'..chat},{text = '✅بله', callback_data = 'ueb:'..chat}
                },{
		...
		local keyboard = {}
							keyboard.inline_keyboard = {
								{
				 {text = '❌خیر', callback_data = 'rtd:'..chat},{text = '✅بله', callback_data = 'cgh:'..chat}
                },{
                   {text = 'بازگشت به منوی اصلی ◀️', callback_data = 'firstmenu:'..chat},{text = 'صفحه قبلی ◀️', callback_data = 'managerlist:'..chat}
				}
							}
		...
		local keyboard = {}
							keyboard.inline_keyboard = {
								{
				 {text = '❌خیر', callback_data = 'njh:'..chat},{text = '✅بله', callback_data = 'alrem:'..chat}
                },{
                   {text = 'بازگشت به منوی اصلی ◀️', callback_data = 'firstmenu:'..chat},{text = 'صفحه قبلی ◀️', callback_data = 'managerlist:'..chat}
				}
							}
		...
		local keyboard = {}
              ...
              edit(q.inline_message_id,'`درخواست شما لغو گردید.`',keyboard)
						...
						if q.data:match('removeowners') then
                           local chat = '-'..q.data:match('(%d+)$')
						   if is_sudo(q.from.id) then
		local keyboard = {}
							keyboard.inline_keyboard = {
								{
				 {text = '❌خیر', callback_data = 'qer:'..chat},{text = '✅بله', callback_data = 'eor:'..chat}
                },{
                   {text = 'بازگشت به منوی اصلی ◀️', callback_data = 'firstmenu:'..chat},{text = 'صفحه قبلی ◀️', callback_data = 'ownerlist:'..chat}
				}
							}
              edit(q.inline_message_id,'هشدار!\n`با انجام این عمل لیست مالکین گروه حذف میگردد.`\n`آیا اطمیان دارید؟`',keyboard)
			   ...
			   Canswer(q.id,"شما دسترسی حذف مالکین گروه را ندارید.\nتنها مالک رسمی ربات قادر به حذف این لیست میباشد.",true)
            end
            end
							------------------------------------------------------------------------
							if q.data:match('eor') then
                           local chat = '-'..q.data:match('(%d+)$')
						   if is_sudo(q.from.id) then
		...
		local keyboard = {}
							keyboard.inline_keyboard = {
								{
				 {text = '❌خیر', callback_data = 'acv:'..chat},{text = '✅بله', callback_data = 'uik:'..chat}
                },{
		...
		local keyboard = {}
							keyboard.inline_keyboard = {
								{
				 {text = '❌خیر', callback_data = 'wyp:'..chat},{text = '✅بله', callback_data = 'bhy:'..chat}
                },{
                   {text = 'بازگشت به منوی اصلی ◀️', callback_data = 'firstmenu:'..chat},{text = 'صفحه قبلی ◀️', callback_data = 'banlist:'..chat}
				}
							}
		...
		local keyboard = {}
							keyboard.inline_keyboard = {
								{
				 {text = '❌خیر', callback_data = 'dfr:'..chat},{text = '✅بله', callback_data = 'vyt:'..chat}
                },{
                   {text = 'بازگشت به منوی اصلی ◀️', callback_data = 'firstmenu:'..chat},{text = 'صفحه قبلی ◀️', callback_data = 'silentlistusers:'..chat}
				}
							}
		...
		local keyboard = {}
							keyboard.inline_keyboard = {
								{
				 {text = '❌خیر', callback_data = 'qre:'..chat},{text = '✅بله', callback_data = 'mlk:'..chat}
                },{
